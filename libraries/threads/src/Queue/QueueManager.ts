...
import type { ThreadPool } from "../Threads/ThreadPool.js";
...
export class QueueManager<T> {
  ...
  constructor(
    ...
    public _threadPool: ThreadPool,
    public getQueueKey: ((data: T) => string) | null = null
  ) {}

  private __getQueueKey(data: any) {
  ...
  private __getQueueData(id: string) {
  ...
  addQueue(queueId: string | number) {
    ...
    for (const thread of this._threadPool.getThreads()) {
  ...
  private _getSyncId(queueId: string | number) {
