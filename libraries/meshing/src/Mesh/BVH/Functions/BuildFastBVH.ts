import { Vector3, BVH_FlatNode } from "./shared";
...
let parentList: any[] = [];
let currentList: Uint32Array, aabb_array_copy: any[];
let k, value, side0, side1, side2;
let bestSplit, goodSplit, okaySplit;
let bestAxis, goodAxis, okayAxis;
let leftWorkCount = 0;
let rightWorkCount = 0;
let currentMinCorner = new Vector3();
let currentMaxCorner = new Vector3();
let testMinCorner = new Vector3();
let testMaxCorner = new Vector3();
let testCentroid = new Vector3();
let currentCentroid = new Vector3();
let spatialAverage = new Vector3();
...
function BVH_Create_Node(
  ...
  if (workList.length < 1) {
    ...
    spatialAverage
      .copy(currentMinCorner)
      .add(currentMaxCorner)
      .multiplyScalar(0.5);
...
export default function BVH_Build_Iterative(
  ...
  const finalArray = new Float32Array(buildnodes.length * 8);
 
  // Copy the buildnodes array into the aabb_array
  for (let n = 0; n < buildnodes.length; n++) {
    // slot 0
    finalArray[8 * n + 0] = buildnodes[n].idPrimitive; // r or x component
    finalArray[8 * n + 1] = buildnodes[n].minCorner.x; // g or y component
    finalArray[8 * n + 2] = buildnodes[n].minCorner.y; // b or z component
    finalArray[8 * n + 3] = buildnodes[n].minCorner.z; // a or w component

    // slot 1
    finalArray[8 * n + 4] = buildnodes[n].idRightChild; // r or x component
    finalArray[8 * n + 5] = buildnodes[n].maxCorner.x; // g or y component
    finalArray[8 * n + 6] = buildnodes[n].maxCorner.y; // b or z component
    finalArray[8 * n + 7] = buildnodes[n].maxCorner.z; // a or w component
  }
  return finalArray;
